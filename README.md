...
I have gone for matched pairs for `“`...`”` and `‘`...`’`. Why not? It would make fonts show start/end quotes much more clearly as a result. An alternative would be matching inverting `!` and `?` maybe. Tricky one.
